/**
 *  Hono MCP middleware のコードをベースにステートフルMCP & サーバーレス環境でも動くように改修しています
 *  from: https://github.com/honojs/middleware/blob/ab5fd8ed8a4daf22a3af8dc9b68a7659a57abfee/packages/mcp/src/index.ts
 */
...
export class StreamableHTTPTransport implements Transport {
  ...
  private async handleGetRequest(ctx: Context) {
    try {
      ...
      return streamSSE(ctx, async (stream) => {
        const resolvedStreamId = typeof streamId === "string"
          ? streamId
  ...
  private async handlePostRequest(ctx: Context, parsedBody?: unknown) {
    try {
      ...
      if (!isInitializationRequest) {
        if (
          !this.#initialized && this.sessionId && this.#onsessioninitialized
        ) {
          this.#onsessioninitialized(this.sessionId);
        }
  ...
  async send(
    ...
    if (isJSONRPCResponse(message) || isJSONRPCError(message)) {
      ...
      const relatedIds = Array.from(this.#requestToStreamMapping.entries())
        .filter(([, streamId]) =>
          this.#streamMapping.get(streamId) === response
        )
        .map(([id]) => id);
